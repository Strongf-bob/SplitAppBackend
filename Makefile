...
.PHONY: setup test lint format-check security-audit run run-dev stop status logs
...
security-audit:
	@if [ -x "$(VENV_PYTHON)" ]; then \
		$(VENV_PYTHON) -m pip_audit -r requirements.txt --timeout 60; \
	else \
