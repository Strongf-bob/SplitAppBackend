...
| Проверка | Команда / условие запуска | Что доказывает | Источник |
|---|---|---|---|
...
| Страница | Связь |
|---|---|
| [Архитектура](Architecture#ключевые-пути) | Контрактные пути, которые покрывают тесты. |
| [Аутентификация и безопасность](Authentication-And-Security#авторизация-ресурсов) | Сценарии безопасности для регрессионных тестов. |
| [Операции и деплой](Operations-And-Deployment#развёртывание) | Проверки CI и проверка работоспособности в production. |
| [Руководство по API](API-Guide) | API-контракт, который надо сохранять синхронным. |
