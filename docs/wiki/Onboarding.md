...
flowchart TD
  ...
  B -->|"Понять продукт и риски"| C["Executive guide"]
  B -->|"Сформировать сценарий и критерии"| D["Product manager guide"]
  B -->|"Внести первое изменение"| E["Contributor guide"]
  B -->|"Принять техническое решение"| F["Staff engineer guide"]
  C --> G["Product overview и User journey"]
  D --> G
  E --> H["Local setup, API guide и Testing and CI"]
  F --> I["Architecture, Data model и Operations"]
```
...
- Backend — источник истины для membership, прав, балансов и платежей; клиентский интерфейс не может заменять серверную проверку. [Project overview](Project-Overview.md#важные-инварианты)
