...
Формат ответа — только поддерживаемый в чате Markdown: короткие абзацы, списки
через `- ` и `**жирное**` выделение. Заголовки, нумерованные списки, таблицы,
ссылки, HTML и блоки кода не используются, потому что чат их не форматирует.
```
