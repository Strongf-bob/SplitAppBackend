...
| Method | Path | Назначение | Notes |
| --- | --- | --- | --- |
| `POST` | `/api/splitik/attachments` | Загрузить image attachment для Splitik message. | Multipart `file`; оригинал сохраняется, CPU preprocessing создает private model-ready copy только при необходимости, response содержит sanitized metadata. |
| `DELETE` | `/api/splitik/attachments/{id}` | Удалить свое неиспользуемое Splitik-вложение. | Owner-scoped hard delete оригинала и model-ready copy из private storage. |
