---
title: "Руководство contributor"
description: "Практический технический маршрут от локального запуска до безопасного изменения SplitApp Backend."
---
...
1. Прочитайте [Обзор проекта](Project-Overview) и [Архитектуру](Architecture), чтобы не смешать router, schema и service responsibility.
2. Выполните [Локальный запуск](Local-Setup); он описывает `.env`, MongoDB и PWA-сборку. Базовые команды определены в [Makefile:13-44](https://github.com/Strongf-bob/SplitAppBackend/blob/main/Makefile#L13-L44).
...
4. Для API-задачи начните со [Справочника API](API-Reference) и [openapi.yaml](https://github.com/Strongf-bob/SplitAppBackend/blob/main/openapi.yaml), а затем найдите router и service по предметной области.
...
2. Проверьте, затрагивает ли изменение lifecycle: soft-deleted ресурсы не должны возвращаться в API, а закрытое событие отклоняет финансовые мутации. [Модель данных](Data-Model.md#состояния-и-soft-delete)
...
4. Добавьте либо обновите regression tests для нового разрешённого и запрещённого пути. Матрица обязательных фокусов есть в [Тестах и CI](Testing-And-CI.md#регрессионные-контракты).
...
| Page | Relationship |
|---|---|
| [Локальный запуск](Local-Setup) | Окружение и первый run. |
