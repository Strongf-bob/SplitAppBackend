...
- Participants/invites: `POST /api/events/{id}/participants`,
  ...
  `POST /api/events/{id}/nearby-code`,
  `GET /api/nearby-invites/{code}/preview`,
  `POST /api/nearby-invites/{code}/accept`.
...
- deep links для invite/nearby flows должны открывать PWA и вести в нужный
  экран после login;
...
- Invite link и nearby code работают end-to-end.
