...
При первом входе после авторизации PWA должна показывать permission onboarding
для доступов, которые нужны рабочим сценариям приложения:

- контакты — только через явный выбор пользователя; PWA не делает скрытый
  background import адресной книги;
- камера — для фото чека из PWA;
- галерея — через системный file picker для выбора готового фото чека;
- уведомления — для будущих напоминаний по событиям, платежам и долгам.

PWA должна учитывать ограничения браузеров: если отдельный API недоступен или
пользователь отказал, приложение продолжает работать и показывает понятный
статус вместо бесконечного повторного запроса.
